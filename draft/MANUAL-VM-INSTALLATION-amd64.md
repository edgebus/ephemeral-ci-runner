...
| Backing file                                                              | Description                               |                                        |
| ------------------------------------------------------------------------- | ----------------------------------------- | -------------------------------------- |
| `qemu-builder-amd64-vm.qcow2`                                             | Base system with all software on board    | base image                             |
| `qemu-builder-amd64-vm-24.qcow2` (based on `qemu-builder-amd64-vm.qcow2`) | Woodpecker Agent VM 24 on tw02            | role=builder, builder.type=osfordev    |
| `qemu-builder-amd64-vm-25.qcow2` (based on `qemu-builder-amd64-vm.qcow2`) | Drone VM 25 on tw02                       | role=builder, builder.type=osfordev    |
| `qemu-builder-amd64-vm-32.qcow2` (based on `qemu-builder-amd64-vm.qcow2`) | Woodpecker Agent VM 32 on evolaptop       | role=builder, builder.type=osfordev    |
| `qemu-builder-amd64-vm-41.qcow2` (based on `qemu-builder-amd64-vm.qcow2`) | Woodpecker Agent VM 41 on lo00.zxteam.net | role=builder, builder.type=docker-host |
| `qemu-builder-amd64-vm-42.qcow2` (based on `qemu-builder-amd64-vm.qcow2`) | Woodpecker Agent VM 42 on lo00.zxteam.net | role=builder, builder.type=osfordev    |
...
docker run --rm --interactive --tty --platform linux/amd64 \
  ...
  theanurin/gentoo-sources-bundle:5.15.167
...
make -j$(nproc)
cp /cache/amd64/arch/x86_64/boot/bzImage /data/qemu-builder-amd64-vm.vmlinuz
exit
```

Copy `.build/qemu-builder-amd64-vm.vmlinuz` to VM Host `/tmp/qemu-builder-amd64-vm.vmlinuz`
...
1. Create image
    ```shell
    dd if=/dev/zero of=/tmp/qemu-builder-amd64-vm.raw bs=1M count=$((4 * 1024))
    mkfs.ext4 -L system -N 2359296 /tmp/qemu-builder-amd64-vm.raw
    mkdir /mnt/gentoo
    mount /tmp/qemu-builder-amd64-vm.raw /mnt/gentoo
    ```
...
1. Copy kernel (this file is not really used, just for convenience/integrity)
    ```shell
    cp /tmp/qemu-builder-amd64-vm.vmlinuz /mnt/gentoo/boot/vmlinuz
    ```
1. Umount
    ```shell
    umount /mnt/gentoo
...
1. Convert image to qcow2 format
    ```shell
    qemu-img convert -O qcow2 /tmp/qemu-builder-amd64-vm.raw /tmp/qemu-builder-amd64-vm.qcow2
    rm /tmp/qemu-builder-amd64-vm.raw
    qemu-img resize /tmp/qemu-builder-amd64-vm.qcow2 48G
    ```
1. Move image to `/opt/qemu-builder-vm/qemu-builder-amd64-vm.qcow2`
    ```shell
    mv /tmp/qemu-builder-amd64-vm.qcow2 /opt/qemu-builder-vm/qemu-builder-amd64-vm.qcow2
    ```
...
1. Run qemu VM
    ```shell
    cp /tmp/qemu-builder-amd64-vm.vmlinuz /opt/qemu-builder-vm/

    export DRIVE_FILE=/opt/qemu-builder-vm/qemu-builder-amd64-vm.qcow2
    export KERNEL_PATH=/opt/qemu-builder-vm/qemu-builder-amd64-vm.vmlinuz
    ...
    qemu-system-x86_64 \
        ...
        -netdev "user,id=vmnic,hostname=qemu-builder-vm-setup,hostfwd=tcp:0.0.0.0:65425-:22" \
...
- Remotely
    ```shell
    cssh -p <bypassed port> root@127.0.0.1
    ```

An alias `cssh` looks like
...
1. Install Drone Exec Runner (see https://gitea.zxteam.net/zxteam/zxteam/src/branch/docs/drone-ci.md)
    ...
    ```
1. Auto-start Docker (not relevant for OS For Dev)
    ```shell
...
export DRIVE_BACKING_FILE=/opt/qemu-builder-vm/qemu-builder-amd64-vm.qcow2
export DRIVE_INSTANCE_FILE="/opt/qemu-builder-vm/qemu-builder-amd64-vm-${INSTANCE_ID}.qcow2"
...
```shell
export INSTANCE_ID="25"
cat <<EOF > /opt/qemu-builder-vm/qemu-builder-amd64-vm-${INSTANCE_ID}.conf
USE_KVM=yes
CPU=6
#CPU_FLAGS=max
MEM=12
...
1. Make first (manual) launch
    ```shell
    INSTANCE_ID="25"
    ln --symbolic /opt/qemu-builder-vm/qemu-builder-amd64-vm /etc/local.d/qemu-builder-amd64-vm.${INSTANCE_ID}.start
    DEBUG=yes VNC_DISPLAY=0 NO_SNAPSHOT=yes /etc/local.d/qemu-builder-amd64-vm.${INSTANCE_ID}.start
    ```
...
1. Set hostname
    ```shell
    export INSTANCE_ID="25"
    echo "hostname=\"qemu-docker-swarm-demo-${INSTANCE_ID}\"" >  /etc/conf.d/hostname
    echo "127.0.0.1 qemu-docker-swarm-demo-${INSTANCE_ID}"    >> /etc/hosts
    echo "::1       qemu-docker-swarm-demo-${INSTANCE_ID}"    >> /etc/hosts
    ```
...
1. In *[Woodpecker Admin UI](https://woodpecker.zxteam.net/admin#agents)* create an agent in **Disabled State** (to prevent start any pipelines)
...
1. cleanup and halt VM `rm /root/.bash_history; /usr/bin/shutdown -hP now; history -c; exit`
1. In *Woodpecker Admin UI* enable the agent
...
1. edit `/etc/drone-runner-exec/osfordev` to
    ...
    - add labels `DRONE_RUNNER_LABELS`, for example `hostname:qemu-builder-amd64-vm-24`
...
1. To troubleshoot problems, begin from `tail --follow /var/log/drone-runner-exec.*` inside Instance VM
  ...
  574  ip link add  qemu_docker_swarm_demo_bridge type bridge
  575  ip link set qemu_docker_swarm_demo_bridge up
  576  ip tuntap add swarm_demo_tap0 mode tap
  577  ip tuntap add swarm_demo_tap1 mode tap
  578  ip tuntap add swarm_demo_tap2 mode tap
  579  ip link set swarm_demo_tap0 up promisc on
  580  ip link set swarm_demo_tap1 up promisc on
  581  ip link set swarm_demo_tap2 up promisc on
  582  ip link set swarm_demo_tap0 master swarm_demo_br0
  583  ip link set swarm_demo_tap1 master swarm_demo_br0
  584  ip link set swarm_demo_tap2 master swarm_demo_br0
