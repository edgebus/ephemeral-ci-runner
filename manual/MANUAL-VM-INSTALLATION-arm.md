...
docker run --rm --interactive --tty \
  ...
  --env KCONFIG_CONFIG=/gentoo-overlay/profiles/qemu-guest/builder/arm32v7/kernel.config \
  ...
  theanurin/gentoo-sources-bundle:6.12.58

# Inside Container
make -j$(nproc)
cp "${KCONFIG_CONFIG}"                  /data/arm7a_hardfp.config
cp /cache/arm32v7/arch/arm/boot/zImage  /data/arm7a_hardfp.vmlinuz
exit
```

See for kernel at `releases/arm7a_hardfp/next/arm7a_hardfp.vmlinuz`
...
cp /data/arm7a_hardfp.vmlinuz /mnt/gentoo/boot/vmlinuz
...
qemu-img convert -O qcow2 /arm7a_hardfp.raw releases/arm7a_hardfp/next/arm7a_hardfp.qcow2
...
# fetch Portage repository "gentoo"
emerge-webrsync
# setup locales
...
# install `git` (to be able to fetch `osfordev` repo)
...
# select profile
eselect profile set osfordev:qemu-guest/builder/arm32v7
# accept keywords
...
rm /usr/bin/shutdown -hP now; history -c; exit
